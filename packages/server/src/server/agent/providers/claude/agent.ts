    ...
    this.query = claudeQuery(
      { prompt: input.iterable, options },
      {
        ...
        onChildProcess: (child) => {
          ...
          // handleRuntimeExit only acts on idle exits (it defers to the query pump
          // throwing while a turn is active). On Windows, orphaned MCP children keep
          // the stdout pipe open so the pump never sees EOF and never throws, so we
          // also need this independent grace-window watcher to fail active turns.
